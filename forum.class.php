...
{
	$query = $this->dbh->prepare('SELECT * FROM `article` ORDER BY `article_id` ASC');
	try
	{
		$query->execute();
	}
...
{
	$query = $this->dbh->prepare('SELECT * FROM `replies` WHERE `topicid`= ? ORDER BY `created`');
	$query->bindValue(1, $topicid);
	try
	{
		$query->execute();
...
/* function that gathers information about the forum within the topic. */
public function forumData($forumId) 
{
	$query = $this->dbh->prepare('SELECT * FROM `article` WHERE `article_id`= ?');
	$query->bindValue(1, $forumId);
	try {
		$query->execute();
		return $query->fetch();
...
{
	$query = $this->dbh->prepare('UPDATE `article` SET `article_lastpost`=? WHERE `article_id`=? ');
	$query->bindValue(1, $username);
