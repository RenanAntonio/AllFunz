...
public function getForum() {
	$query = $this->dbh->prepare('SELECT * FROM `article` ORDER BY `id` ASC');
	try
	{
		$query->execute();
	}
...
public function getTopics($forumid) {
	...
	$query = $this->dbh->prepare('SELECT * FROM `topics` WHERE `forum` = (SELECT `id` FROM `article` WHERE `name` = ?)');
...
public function addTopic($username, $title, $message, $whatforum, $created) {
	$query = $this->dbh->prepare('INSERT INTO `topics` (`starter`, `title`, `message`, `forum`, `created`) VALUES (?, ?, ?, (SELECT `id` FROM `article` WHERE `name` = ?), ?) ');
...
public function topicData($topicName, $topicId) {
	$query = $this->dbh->prepare('SELECT * FROM `topics` WHERE `forum` = (SELECT `id` FROM `article` WHERE `name` = ?) AND `id` = ?');
	$query->bindValue(1, $topicName);
...
{
	$query = $this->dbh->prepare('UPDATE `article` SET `lastpost`=? WHERE `id`=? ');
	$query->bindValue(1, $whatforum);
	try
	{
		$query->execute();
	}
