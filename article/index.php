...
//Collects Main Banner and Picture
    ...
    $snoopy = new Snoopy();
    $poster = file_get_contents("https://www.bing.com/images/search?q=".$tagBanner."+poster&qft=+filterui:aspect-tall+filterui:imagesize-large");
    $picture = file_get_contents("https://www.bing.com/images/search?&q=".$tagBanner."&qft=+filterui:imagesize-medium+filterui:aspect-tall&FORM=R5IR29&filt=custom");
...
<head>
    ...
    <title><?php echo $rows1["article_name"]. ' | AllFunz' ?></title>
...
<body class="articlePage" page-id="<?php echo $rows1["article_id"]; ?>" page-category="<?php echo $rows1["article_category"]; ?>" page-type="<?php echo $rows1["article_page_type"]; ?>">
    ...
    <div class="hiddenID"><?php echo $article_id; ?></div>
    ...
    <div class="mainBanner">
        <div class="article info">
            <div class="article picture" style="background-image: url('<?php echo $rows1["article_picture"]; ?>');"></div>
    ...
    <div id="forum">
        ...
        if ($_GET['article-id']) {
            if (count($topics) == 0) {
                ...
                echo '<div class="msg">There are no topics created for this article. Be the first one and share it with your friends.</div>';
                echo '<div class="msgBt btEmpty">Create a Topic</div>';
                echo '</div>';
                ?>
                <?php
            } else {
                ...
                echo '<div class="wrapTopics"><ul>';
                    foreach ($topics as $topic) {
                        ...
                        echo '<li><a href="../../topic.php?id='.$topic_id.'">'.$topic_title.'</a><p>Last post by '.$topic_lastpost.'</p></li>';
                    }
                    echo '<div class="msgBt btEmpty">Post new topic</div>';
                echo '</ul></div>';
            }
        }
        ?>
    ...
    <script>
        ...
        $('.btEmpty').on('click', function(){
            window.location.href = "../../addtopic.php?forum="+ $('.hiddenID').text();
        });
    </script>
